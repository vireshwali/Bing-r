...
@QmlElement
class CountryFilterViewModel(QAbstractListModel):
    ...
    async def _doFetch(self, savedValue: str) -> None:
        if not self._service:
            return
        try:
            pairs = await self._service.getDistinctCountries()
            items = [FilterOptionModel(value="all", text="All Countries")]
            items.extend(FilterOptionModel(value=code, text=name) for code, name in pairs)
            self._replaceItems(items, savedValue)
        except Exception as e:
            logger.exception("Failed to fetch countries: %s", e)
        finally:
